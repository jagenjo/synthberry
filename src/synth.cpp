...
void Synth::generateAudio( float* buffer, int len, AudioSpec& spec )
{
	//synth
...
void Synth::generateOscillator(Oscillator& osc, AudioSpec& spec)
{
...
Sample* Synth::loadSample(std::string filename)
{
...
Synth::SamplePlayback* Synth::playSample( Sample* sample, float volume, bool loop )
{
	//find free sample
	int i = 0;
...
bool Synth::updateSamplesBuffer( AudioSpec& spec )
{
	bool playing = false;
