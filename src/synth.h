...
class Synth {
	...
	public:
		...
		void generateAudio( float* buffer, int len, AudioSpec& spec );
		void generateOscillator( Oscillator& osc, AudioSpec& spec);
		void applyFilter( Oscillator& osc, AudioSpec& spec);
		...
		bool updateSamplesBuffer( AudioSpec& spec );
